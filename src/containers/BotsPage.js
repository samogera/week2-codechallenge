...
class BotsPage extends Component {
  ...
  componentDidMount() {
    fetch("https://db-json-e59m.onrender.com/bots")
  ...
  removeBotPermanently = (bot) => {
    let newCollection = this.state.botCollection.filter(
      (card) => card !== bot
    );
    ...
    fetch(`https://db-json-e59m.onrender.com/bots/${bot.id}`, {
      method: "DELETE",
    })
      .then((response) => response.json())
    ...
    return (
      <div>
        ...
        {collectionVisible ? (
          ...
          <BotSpecs bot={botSpecs} back={this.displayBotCollection} enlist={this.addToArmy} />
        )}
      </div>
    );
  }
